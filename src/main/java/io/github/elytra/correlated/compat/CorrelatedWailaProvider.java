...
public class CorrelatedWailaProvider implements IWailaDataProvider {
	...
	public NBTTagCompound getNBTData(EntityPlayerMP player, TileEntity te, NBTTagCompound nbt, World world, BlockPos pos) {
		...
		if (te instanceof TileEntityNetworkMember) {
			...
			nbt.setInteger("MaxEnergyPerTick", Correlated.inst.controllerCap);
	...
	public List<String> getWailaBody(ItemStack stack, List<String> body, IWailaDataAccessor access, IWailaConfigHandler config) {
		...
		if (access.getBlock() == Correlated.controller) {
			if (nbt.hasKey("ErrorReason")) {
				body.add("\u00A7c"+I18n.format("tooltip.correlated.controller_error."+nbt.getString("ErrorReason")));
			} else if (nbt.hasKey("BootTicks") && nbt.getInteger("Energy") >= nbt.getInteger("EnergyPerTick")) {
				int bootTicks = nbt.getInteger("BootTicks");
				if (bootTicks < 0) {
					body.add("\u00A7a"+I18n.format("tooltip.correlated.controller_booting.hard"));
				} else {
					body.add("\u00A7a"+I18n.format("tooltip.correlated.controller_booting"));
				}
				int seconds;
				if (bootTicks >= 0) {
				...
				if (seconds == 1) {
					body.add("\u00A7a"+I18n.format("tooltip.correlated.controller_boot_eta_one"));
				} else {
					body.add("\u00A7a"+I18n.format("tooltip.correlated.controller_boot_eta", seconds));
				}
			}
			body.add(I18n.format("tooltip.correlated.controller_consumption_rate", nbt.getInteger("EnergyPerTick"), nbt.getInteger("MaxEnergyPerTick")));
			body.add(I18n.format("tooltip.correlated.controller_energy_buffer", nbt.getInteger("Energy"), nbt.getInteger("MaxEnergy")));
		} else if (access.getTileEntity() instanceof TileEntityNetworkMember) {
			if (nbt.getBoolean("HasController")) {
				body.add(I18n.format("tooltip.correlated.member_consumption_rate", nbt.getInteger("EnergyPerTick")));
			} else {
				body.add("\u00A7c"+I18n.format("tooltip.correlated.no_controller"));
			}
		}
		if (access.getTileEntity() instanceof TileEntityDriveBay) {
			body.add(I18n.format("tooltip.correlated.drive_count", nbt.getInteger("DriveCount")));
			body.add(I18n.format("tooltip.correlated.bytes_used", Numbers.humanReadableBytes(nbt.getInteger("BytesUsed")*1024), Numbers.humanReadableBytes(nbt.getInteger("MaxBytes")*1024), nbt.getInteger("BytesPercent")));
			...
			body.add(I18n.format("tooltip.correlated.memory_count", nbt.getInteger("MemoryCount")));
			body.add(I18n.format("tooltip.correlated.bytes_available", Numbers.humanReadableBytes(nbt.getInteger("MaxBytes"))));
		}
		return body;
	}
	...
	public ItemStack getWailaStack(IWailaDataAccessor access, IWailaConfigHandler config) {
		...
		} else if (access.getBlock() == Correlated.wireless_endpoint) {
