...
import io.github.elytra.correlated.network.SetGlitchingStateMessage.GlitchState;
		...
		if (!worldObj.isRemote && worldObj.getBlockState(getPosition()).getBlock() == Blocks.PORTAL) {
			if (getStack() != null && getStack().getItem() == Correlated.misc) {
				if (getStack().getMetadata() == 3) {
					worldObj.playSound(null, posX, posY, posZ, Correlated.convert, SoundCategory.PLAYERS, 1, rand.nextFloat()+0.75f);
					if (worldObj instanceof WorldServer) {
						((WorldServer)worldObj).spawnParticle(EnumParticleTypes.REDSTONE, posX, posY, posZ, 100, 0.2, 0.2, 0.2, 100);
					}
					setStack(new ItemStack(Correlated.misc, getStack().stackSize, 6));
					noTeleport = true;
