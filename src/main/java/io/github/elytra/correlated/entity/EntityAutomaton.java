...
import io.github.elytra.correlated.entity.ai.EntityAIAutomatonAttackRangedBow;
import io.github.elytra.correlated.entity.ai.EntityAIAutomatonFollowOwner;
import io.github.elytra.correlated.entity.ai.EntityAIAutomatonOwnerHurtByTarget;
import io.github.elytra.correlated.entity.ai.EntityAIAutomatonOwnerHurtTarget;
...
import io.github.elytra.correlated.storage.ITerminal;
import io.github.elytra.correlated.storage.SimpleUserPreferences;
import io.github.elytra.correlated.storage.UserPreferences;
...
public class EntityAutomaton extends EntityCreature implements IEntityOwnable, ITerminal, IDigitalStorage {
    ...
    protected boolean processInteract(EntityPlayer player, EnumHand hand, ItemStack stack) {
        if (stack != null && stack.getItem() == Correlated.weldthrower) return false;
        if (stack != null && stack.getItem() == Correlated.misc && stack.getMetadata() == 5) {
        ...
        if (!worldObj.isRemote) {
            if (getHealth() < 1) {
                player.addChatComponentMessage(new TextComponentTranslation("msg.correlated.automaton_dead"));
            } else if (!isOwner(player)) {
                if (getOwnerId() == null) {
                    player.addChatComponentMessage(new TextComponentTranslation("msg.correlated.automaton_untamed"));
                } else if (isAngry()) {
                    player.addChatComponentMessage(new TextComponentTranslation("msg.correlated.automaton_angry"));
                } else {
                    player.addChatComponentMessage(new TextComponentTranslation("msg.correlated.automaton_not_yours"));
                }
            } else {
                if (player.isSneaking()) {
                    ...
                    if (getItemStackFromSlot(EntityEquipmentSlot.CHEST) != null) {
                        ...
                        slot = null;
                        player.openGui(Correlated.inst, 4, worldObj, getEntityId(), 0, 0);
                    }
                    if (slot != null) {
